...
function init()
  ...
  ----UNDERLAY----
  frameFrac = 0
  aviTexture = AviTexture("movies-data/dragon128.avi",256,256)
  addToUnderlay(aviTexture)
  ----MODEL----
  ...
  flagMaterial:setDiffuseTexture(aviTexture) -- AviTexture
...
function update()
  ...
  frameFrac = frameFrac+simTime*aviTexture:getFrameRate()
  if frameFrac > 1 then
    local frame = aviTexture:getCurrentFrame()+frameFrac
    frameFrac = frameFrac-math.floor(frameFrac)
    if frame >= aviTexture:getFramesCount() then
      frame = 0
    end
    aviTexture:setCurrentFrame(frame)
  end
